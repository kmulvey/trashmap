...
func InsertArea(db *pgx.Conn, schema string, userID int64, polygon string) error {
	...
	var _, err = db.Exec(context.Background(), stmt, userID)
	return err
}

// GetAreaID returns the id of an area given userID and the area
func GetAreaID(db *pgx.Conn, schema string, userID int64, polygon string) (int64, error) {
	// we use fmt.Sprintf to fill in the polygon string because the sql driver
	// cant see variables within '', which are required by ST_GeometryFromText.
	// The polygon value has been processed quite a bit by this point anyway.
	var stmt = fmt.Sprintf(`SELECT id FROM %s.areas where user_id=$1 and polygon=ST_GeometryFromText('POLYGON((%s))')`, schema, polygon)
	var id int64
	var err = db.QueryRow(context.Background(), stmt, userID).Scan(&id)
	if err != nil {
		return -1, err
	}
...
func DeleteArea(db *pgx.Conn, schema string, userID int64) error {
...
func GetPickupAreasWithinArea(db *pgx.Conn, schema string, polygon string) ([]string, error) {
	// we use fmt.Sprintf to fill in the polygon string because the sql driver
	// cant see variables within '', which are required by ST_GeometryFromText.
	// The polygon value has been processed quite a bit by this point anyway.
	...
	var rs, err = db.Query(context.Background(), stmt)
	if err != nil {
		return nil, err
	}
